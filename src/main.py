    ...
    if mode == "test": # test模式写入，train模式忽略
        ...
        if os.path.isfile(filename) == False:# 如果文件不存在，则创建
            with open (filename,'w', newline='') as f:
                # 写入列名
                fieldnames=['encoder','opn','pre_type','use_llm_prior','use_cl','gpu','datetime','cl_weight',
                            'train_len','test_len','temperature','tau_hard','lr','n_hidden',
        ...
        with open (filename,'a', newline='') as f:
            writer = csv.writer(f)
            row={'encoder':args.encoder,'opn':args.opn,'pre_type':args.pre_type,'use_llm_prior':args.use_llm_prior,'use_cl':args.use_cl,'gpu':args.gpu,'datetime':datetime.now(),'cl_weight':args.cl_weight,
                'train_len':args.train_history_len,'test_len':args.test_history_len,'temperature':args.temperature,'tau_hard':args.tau_hard,'lr':args.lr,'n_hidden':args.n_hidden,
...
def run_experiment(args, n_hidden=None, n_layers=None, dropout=None, n_bases=None):
    ...
    # Deterministic checkpoint name so that --test can re-load the trained model
    # (the original baseline appended str(time.time()), which broke --test).
    # Pass --checkpoint to override the auto-generated path.
    model_name = "{}-len{}-gpu{}-lr{}-tau{}-tauh{}-cl{}-clw{}-pre{}-h{}-{}"\
        .format(args.dataset, args.train_history_len, args.gpu, args.lr, args.temperature, args.tau_hard,
                args.use_cl, args.cl_weight, args.pre_type, args.n_hidden, args.encoder)
    model_state_file = args.checkpoint if args.checkpoint else ('./models/' + model_name + ".pt")
    ...
    # Module 1 (LLM-driven semantic prior injection) replaced the e-w-graph
    # static branch. We no longer build a static word/entity graph here; instead
    # the model loads the offline pre-computed text embeddings produced by
    # data/encode_llm_prior.py.
    static_graph = None
    llm_emb_dir = args.llm_emb_dir or os.path.join("./data", args.dataset)
    ...
    model = RecurrentRGCN(args.decoder,
                        ...
                        llm_text_dim=args.llm_text_dim,
    ...
    if args.test and os.path.exists(model_state_file):
        ...
        raise FileNotFoundError(
            "Checkpoint not found at {}. Run training first or pass --checkpoint <path>.".format(model_state_file))
    else:
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                ...
                for id in range(2):
                    ...
                    # Module 4: L_total = L_CE + lambda * L_CL.
                    loss = loss_e + args.cl_weight * loss_cl

                    losses.append(loss.item())
                    losses_e.append(loss_e.item())
                    losses_r.append(loss_r.item())
                    losses_static.append(loss_cl.item())
                    loss.backward()
                    torch.nn.utils.clip_grad_norm_(model.parameters(), args.grad_norm)  # clip gradients
                    optimizer.step()
                    optimizer.zero_grad()
                # break
            print("Epoch {:04d} | Ave Loss: {:.4f} | entity-relation-cl:{:.4f}-{:.4f}-{:.4f} Best MRR {:.4f} | Model {} "
...
if __name__ == '__main__':
    ...
    parser.add_argument("--checkpoint", type=str, default=None,
                        help="explicit path to a saved .pt checkpoint to load when --test")
    ...
    # Module 1: LLM-driven semantic prior injection (replaces the e-w-graph static graph)
    parser.add_argument("--use-llm-prior", action=argparse.BooleanOptionalAction, default=True,
                        help="inject the LLM-derived text-view semantic prior into the 0-th layer features")
    parser.add_argument("--llm-emb-dir", type=str, default=None,
                        help="directory holding entity_text_emb.pt / relation_text_emb.pt; "
                             "defaults to ./data/<dataset>")
    parser.add_argument("--llm-text-dim", type=int, default=1536,
                        help="hidden size of the frozen LLM (Qwen2.5-1.5B = 1536)")
    ...
    parser.add_argument("--temperature", type=float, default=0.07,
                        help="tau in Module 4: temperature for the InfoNCE softmax")
    parser.add_argument("--tau-hard", type=float, default=0.1,
                        help="tau_hard in Module 4: temperature for the adaptive hard-negative weight beta_k")
    parser.add_argument("--cl-weight", type=float, default=0.5,
                        help="lambda in Module 4: trade-off between cross-entropy L_CE and contrastive L_CL")
    # configuration for encoder RGCN stat
    parser.add_argument("--weight", type=float, default=1,
                        help="weight of static constraint")
    parser.add_argument("--pre-weight", type=float, default=0.7,
                        help="(deprecated by Module 4 fusion MLP; kept for argparse compatibility)")
