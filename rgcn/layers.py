...
class UnionRGCNLayer3(nn.Module):
    """Module 3: relation-modulated R-GCN layer for the global subgraph.

    For every edge (u, r', v) in the RWR-sampled subgraph G_sub a vector
    modulation
        m_{r'}^l = MLP([h_{r'}^l || h_r^l])
    is computed where h_r^l is the *per-target-node* query-relation context
    supplied via the DGL ndata field ``q_rel`` (zero for non-query
    entities). The modulation gates the source feature element-wise before a
    shared neighbor projection W is applied:
        h_v^{(l+1)} = sigma(
            sum_{r' in R} sum_{u in N_{r'}^sub(v)} 1/c_{v,r'} * W (h_u^l ⊙ m_{r'}^l)
            + W_0 h_v^l)

    Design notes / simplifications compared to the paper's exact formula:
      * W is shared across relations (same parameterisation as
        ``UnionRGCNLayer2``); relation-specific behaviour comes from the
        per-edge ``rel_emb`` and the modulation MLP.
      * ``c_{v,r'}`` is implemented through the existing degree-based
        ``norm`` edge weight produced by ``build_graph``.
    """

    def __init__(self, in_feat, out_feat, num_rels, num_bases=-1, bias=None,
                 activation=None, self_loop=False, dropout=0.0, skip_connect=False, rel_emb=None):
        super(UnionRGCNLayer3, self).__init__()
        self.in_feat = in_feat
        self.out_feat = out_feat
        self.bias = bias
        self.activation = activation
    ...
    def msg_func(self, edges):
        # h_{r'}^l: per-edge relation embedding.
        rel = self.rel_emb.index_select(0, edges.data['type']).view(-1, self.out_feat)
        # h_r^l: query-relation context attached to each target node v.
        q_rel = edges.dst['q_rel'].view(-1, self.out_feat)
        # m_{r'}^l = MLP([h_{r'}^l || h_r^l]).
        m_mod = self.modulation_mlp(torch.cat([rel, q_rel], dim=1))
        # gated source feature: h_u^l ⊙ m_{r'}^l.
        src = edges.src['h'].view(-1, self.out_feat)
        gated = src * m_mod
        msg = torch.mm(gated, self.weight_neighbor)
        return {'msg': msg}
